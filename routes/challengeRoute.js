...
// 현재 버전
router.post('/challenge/:modeId', (req, res) => {
...
// 현재 버전
router.get('/challenge/:modeId', (req, res) => {
  const { modeId } = req.params;

  challengeUser.set('collection', `challengeuser${modeId}`);
  const ChallengeUser = mongoose.model('ChallengeUser', challengeUser, `challengeusers${modeId}`);
  ChallengeUser.find({}, '-_id ranking name score time ensuredCell movement item1\ item2\ item3\ item4')
